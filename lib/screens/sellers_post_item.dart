...
class _SellerPostItemState extends State<SellerPostItem> {
  ...
  Future<void> _uploadAndPublish() async {
    ...
    try {
      ...
      // await FirebaseFirestore.instance.collection('sellerListings').add({
...
//
//     try {
//       await FirebaseFirestore.instance.collection('sellerListings').doc(itemId).set({
//         'title': titleController.text.trim(),
//         'category': selectedCategory,
//         'price': double.parse(priceController.text.trim()),
//         'isFixed': isFixed,
...
//         'isAvailable': true,
//         'isSold': false,
//         'imageUrl': '', // IMPORTANT: Added a placeholder for imageUrl. You'll need to implement image upload later.
//         // 'isRemoved': false, // REMOVED: Not needed for permanent delete
//       });
//
//       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Item Published')));
//       // Navigate back to dashboard, and ensure data is refreshed
//       Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => SellerDashboard()));
//     } catch (e) {
//       ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Publish failed: $e')));
//     } finally {
...
//   Widget build(BuildContext context) {
//     final double keyboardHeight = MediaQuery.of(context).viewInsets.bottom;
//     const double bottomAppBarHeight = 60.0;
//     const double fabDiameter = 56.0;
//     const double fabNotchMargin = 10.0;
//     final double fixedBottomAreaHeight = bottomAppBarHeight + (fabDiameter / 2) + fabNotchMargin + 10.0;
//
//     return Scaffold(
//       resizeToAvoidBottomInset: false,
//       appBar: AppBar(
