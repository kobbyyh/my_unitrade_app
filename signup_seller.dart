...
class _SellerSignupScreenState extends State<SellerSignupScreen> {
  ...
  Future<void> _sendOtpToEmail() async {
    ...
    if (email.isNotEmpty) {
      ...
      // You can integrate with a service like Firebase Auth or another API here
      try {
        await _auth.sendPasswordResetEmail(email: email);
        _showSuccess("OTP sent to email successfully");
      } catch (e) {
        _showError("Failed to send OTP to email");
      }
    } else {
...
// Method to show success message after signup
        ...
        actions: <Widget>[
          TextButton(
            onPressed: () {
              // Close the dialog
              Navigator.of(ctx).pop();
              // Navigate to the seller dashboard after success
              Navigator.pushReplacementNamed(context, '/sellerDashboard');
            },
            child: Text('OK'),
          ),
